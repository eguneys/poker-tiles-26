...
import { Material, MaterialSide, RemainingChecks, type Setup } from './setup';
import { SquareSet } from './squareSet';
import { COLORS, FILE_NAMES, ROLES, type Color, type Piece, type Square } from './types';
...
export const parseBoardFen = (boardPart: string): Result<Board, FenError> => {
  ...
  for (let i = 0; i < boardPart.length; i++) {
    ...
    if (c === '/' && file === 8) {
      ...
      if (step > 0) file += step;
        ...
        if (file >= 8 || rank < 0) return Result.err();
        const square = file + rank * 8;
        const piece = charToPiece(c);
        if (!piece) return Result.err();
        if (boardPart[i + 1] === '~') {
          piece.promoted = true;
          i++;
        }
  ...
  if (rank !== 0 || file !== 8) return Result.err();
  return Result.ok(board);
};

export const parsePockets = (pocketPart: string): Result<Material, FenError> => {
  if (pocketPart.length > 64) return Result.err();
...
export const parseFen = (fen: string): Result<Setup, FenError> => {
  ...
  if (boardPart.endsWith(']')) {
    ...
    if (pocketStart === -1) return Result.err();
  ...
  return board.chain(board => {
    ...
    if (defined(epPart) && epPart !== '-') {
      ...
      if (!defined(epSquare)) return Result.err();
    }
    ...
    if (!defined(halfmoves)) return Result.err();

    const fullmovesPart = parts.shift();
    const fullmoves = defined(fullmovesPart) ? parseSmallUint(fullmovesPart) : 1;
    if (!defined(fullmoves)) return Result.err();

    const remainingChecksPart = parts.shift();
    let remainingChecks: Result<RemainingChecks | undefined, FenError> = Result.ok(undefined);
    if (defined(remainingChecksPart)) {
      if (defined(earlyRemainingChecks)) return Result.err();
      remainingChecks = parseRemainingChecks(remainingChecksPart);
    } else if (defined(earlyRemainingChecks)) {
      remainingChecks = earlyRemainingChecks;
    }

    if (parts.length > 0) return Result.err();
